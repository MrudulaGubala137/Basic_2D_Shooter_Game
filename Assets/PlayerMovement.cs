...
{[SerializeField]
    ...
    private void MovementOfPlayer(float speedOfPlayer)
    {
        ...
        else if(Input.GetKeyUp(KeyCode.RightArrow) && transform.position.x <= 10.0f)
            transform.Translate(speedOfPlayer, 0, 0);
        else if (Input.GetKeyUp(KeyCode.UpArrow) && transform.position.y <= 4.0f)
            transform.Translate(0,speedOfPlayer,0);
        else if (Input.GetKeyUp(KeyCode.DownArrow) && transform.position.y >= -4.0f)
                transform.Translate(0,-speedOfPlayer, 0);
